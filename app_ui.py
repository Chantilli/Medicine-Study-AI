...
try:
    ...
    def chip_sugerencia(texto, on_click):
        return ft.Container(
            ...
            border_radius=20, padding=ft.Padding(14, 8, 14, 8),
...
try:
    ...
    def configurar_shell_responsive(
        page, sidebar, contenido_principal, layout_principal, ancho_movil=800
    ):
        ...
        def al_redimensionar(e=None):
            if page.width and page.width < ancho_movil:
        ...
        boton_menu = ft.IconButton(
            icon=ft.Icons.MENU, icon_color="#3b82f6",
            tooltip="Mostrar u ocultar menú", on_click=alternar_sidebar,
        )
        page.on_resized = al_redimensionar
        layout_principal.controls = [
            sidebar, ft.VerticalDivider(width=1, color="#1f212a"),
            contenido_principal,
        ]
...
def main(page: ft.Page):
    ...
    def al_completar_subida(e):
        ...
        try:
            ...
            if alertas_inyeccion:
                chat_view.controls.append(
                    tarjeta_aviso(f"🛡️ Filtro de seguridad: {resumir_alertas(alertas_inyeccion)}", tipo="warning")
                )
    ...
    def _enviar_mensaje_interno(texto):
        ...
        if not limite_chat["permitido"]:
            ...
            chat_view.controls.append(tarjeta_aviso(f"🚦 {limite_chat['motivo']}", tipo="warning"))
            page.update()
            return
