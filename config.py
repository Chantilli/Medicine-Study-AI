...
def construir_system_prompt(codigo_idioma: str = IDIOMA_POR_DEFECTO) -> dict:
    ...
    contenido = (
        ...
        "Los números de referencia son identificadores inmutables asignados por el sistema: no los "
        "cambies, no los desplaces, no los reordenes y no crees números nuevos. En la sección de "
        "referencias conserva exactamente la correspondencia ID [n] → paper que aparece en el contexto. "
        "Incluye en la sección de referencias únicamente los papers que hayas citado explícitamente "
        "en el cuerpo de la respuesta con [n]. Si un paper del contexto no fue citado, exclúyelo "
        ...
        "Si el contexto solo contiene [1] y [2], está prohibido escribir [3], [4], [5] o [6], "
        ...
        "del QRS durante el segmento ST y continúa hasta el final de la onda T. Si se distinguen "
        "subfases, la eyección rápida corresponde al segmento ST y la eyección reducida al "
        "inicio de la onda T hasta el final de la onda T. No describas toda la eyección como "
        "si comenzara en la onda T ni omitas el segmento ST. "
